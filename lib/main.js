...
exports.main = function() {
	...
	panel = require("panel").Panel({
				...
				contentScript: 
					...
					"self.port.on('newAddonMessage',newAddonMessage);" +
					"self.port.on('displayMessage',displayMessage);" +
					"self.port.on('removeMessage',removeMessage);" +
					"self.port.on('removeAllMessages',removeAllMessages);" +
					"self.port.on('getNewHeight',resize)",
				width: 400
			});
	panel.port.on("update",update);
	panel.port.on("resize",function(height){ panel.resize(400, height); });
	panel.on("show",function() {panel.port.emit("getNewHeight")});
...
function update() {	
	Request.Request({
		...
		onComplete:
			...
			function (Response) {
				panel.port.emit("removeAllMessages");
				...
				if(apiData) {
					...
					if(apiData.hasOwnProperty("error")) {
						...
						if(apiData.error == 500) {
							...
							panel.port.emit("newAddonMessage","Please log in to view your messages.");
						} else {
							//Some other error occurred.
							widget.contentURL	= ICON_ERROR;
							widget.tooltip		= "Please notify me if the problem persists!";
							panel.port.emit("newAddonMessage","An unknown error occurred! Error code: " + messages.error + " " + messages.message);
						}
					} else {
						...
						if(isEmpty(apiData.offers) && isEmpty(apiData.replies)) {
							//No messages.
							console.log("No new");
							panel.port.emit("newAddonMessage","No new messages.");
							...
							for each (var offer in apiData.offers) {
								for each(var post in offer.posts) {												//Step through all posts in all offers.
									let message =	{															//Create a new JSON object to pass onto the panel and notifications.
														id:				post.postid,
														type:			"offer",
									...
									panel.port.emit("displayMessage",message);									//Write to the panel.
							...
							for each (var reply in apiData.replies) {											//Step through all posts in all replies.
								for each(var post in reply.posts) {
									...
									panel.port.emit("displayMessage",message);
				...
				//Response could not be JSON-serialized or no response at all.
					...
					panel.port.emit("loggedInState",false);
					panel.port.emit("newAddonMessage","An unknown error occured. It's probably a connection problem.");
					widget.contentURL	= ICON_ERROR;
					widget.tooltip		= "Please notify me if the problem persists!";
				}
			}

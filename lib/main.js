...
function update() {
	//This update might be called by the user if the response was not a JSON-serializable string.
	widget.removeListener("click",update);
		...
		onComplete:
			...
			function (Response) {
				...
				if(apiData) {
					//Check for errors.
					if(apiData['error']) {
						//Display the navigationbar for users that are not logged in.
						...
						if(apiData.error == 500) {
							...
							widget.tooltip		= "Error Code: " + messages.error + " " + messages.message;
						}
					} else {
						//Display the navigationbar for usres that are logged in.
						panel.port.emit("loggedIn");
						
						if(isEmpty(apiData.offers) && isEmpty(apiData.replies)) {
							//No messages destroy old messages and move on.
							panel.port.emit("destroyMessages",false)
							widget.contentURL	= ICON_NOMESSAGES;
							widget.tooltip		= "Logged in as " + apiData.user.nickname + ".";
						} else {
							//Unread messages!
							//Destroy old messages
							panel.port.emit("destroyMessages",true);											//Destroy old messages.
							
							var newMessages = false;																		
							var latestOffer = latestMessage;
							
							if(showAllNotifications)															//Add the notifier.js to the current tab.
								var worker = Tabs.activeTab.attach({ contentScriptFile: PAGE_NOTIFIER });
							
							for each (var offer in apiData.offers) {
								for each(var post in offer.posts) {												//Step through all the posts in all the offers.
							...
							for each (var reply in apiData.replies) {											//Step through all the post in all the replies.
							...
							if(showJustOneNotification && newMessages) {										//Tell the user they have new message in only one notification.
							...
							widget.tooltip		= "Click to see the messages.";
							
						}
					}
				//Response could not be JSON-serialized.
				} else {
					widget.contentURL	= ICON_ERROR;
					widget.tooltip		= "Click here to update. TF2Op is probably down.";
					widget.on("click",update)
				}
			}
	}).get();
}
